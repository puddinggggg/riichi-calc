export const WINDS = [
  { value: 'east', label: '동' },
  { value: 'south', label: '남' },
  { value: 'west', label: '서' },
  { value: 'north', label: '북' },
];

export const WIN_TYPES = [
  { value: 'ron', label: '론' },
  { value: 'tsumo', label: '쯔모' },
];

export const PLAYER_TYPES = [
  { value: 'dealer', label: '친' },
  { value: 'nonDealer', label: '자' },
];

export const YAKU_LIST = [
  { id: 'riichi', name: '리치', closedHan: 1, openHan: 0 },
  { id: 'doubleRiichi', name: '더블리치', closedHan: 2, openHan: 0 },
  { id: 'menzenTsumo', name: '멘젠쯔모', closedHan: 1, openHan: 0 },
  { id: 'tanyao', name: '탕야오', closedHan: 1, openHan: 1 },
  { id: 'pinfu', name: '핑후', closedHan: 1, openHan: 0 },
  { id: 'ipeiko', name: '이페코', closedHan: 1, openHan: 0 },
  { id: 'ryanpeko', name: '량페코', closedHan: 3, openHan: 0 },
  { id: 'chiitoi', name: '치또이츠', closedHan: 2, openHan: 0 },
  { id: 'yakuhai', name: '역패', closedHan: 1, openHan: 1 },
  { id: 'toitoi', name: '또이또이', closedHan: 2, openHan: 2 },
  { id: 'sananko', name: '삼암각', closedHan: 2, openHan: 2 },
  { id: 'sanshokuDoujun', name: '삼색동순', closedHan: 2, openHan: 1 },
  { id: 'ittsuu', name: '일기통관', closedHan: 2, openHan: 1 },
  { id: 'sanshokuDouko', name: '삼색동각', closedHan: 2, openHan: 2 },
  { id: 'sanKantsu', name: '삼깡즈', closedHan: 2, openHan: 2 },
  { id: 'shosangen', name: '소삼원', closedHan: 2, openHan: 2 },
  { id: 'chanta', name: '찬타', closedHan: 2, openHan: 1 },
  { id: 'honroutou', name: '혼노두', closedHan: 2, openHan: 2 },
  { id: 'junchan', name: '준찬타', closedHan: 3, openHan: 2 },
  { id: 'honitsu', name: '혼일색', closedHan: 3, openHan: 2 },
  { id: 'chinitsu', name: '청일색', closedHan: 6, openHan: 5 },
  { id: 'kokushi', name: '국사무쌍', closedHan: 13, openHan: 0 },
  { id: 'chuuren', name: '구련보등', closedHan: 13, openHan: 0 },
  { id: 'suuankou', name: '사암각', closedHan: 13, openHan: 0 },
  { id: 'ryuiso', name: '녹일색', closedHan: 13, openHan: 13 },
  { id: 'daisangen', name: '대삼원', closedHan: 13, openHan: 13 },
  { id: 'shosushi', name: '소사희', closedHan: 13, openHan: 13 },
  { id: 'daisushi', name: '대사희', closedHan: 13, openHan: 13 },
  { id: 'suukantsu', name: '사깡즈', closedHan: 13, openHan: 13 },
  { id: 'tsuiso', name: '자일색', closedHan: 13, openHan: 13 },
  { id: 'chinroto', name: '청노두', closedHan: 13, openHan: 13 },
];

export const LIMITS = [
  { key: 'mangan', label: '만관' },
  { key: 'haneman', label: '하네만' },
  { key: 'baiman', label: '배만' },
  { key: 'sanbaiman', label: '삼배만' },
  { key: 'yakuman', label: '역만' },
];
